...
genes <- read.table("/mnt/data/ORF_Genes_names_processed_update_2019.tsv",
                    header = TRUE, sep = "\t", stringsAsFactors = FALSE)

features <- read.table("/mnt/data/AllGene_ChromosomalFeatures_with_description.tsv",
                       header = TRUE, sep = "\t", stringsAsFactors = FALSE)
...
genes_selecionados <- genes[, c("Gene.Name", "ORF")]
colnames(genes_selecionados) <- c("GeneID", "ORF_ID")  # Renomeando colunas

features_selecionados <- features[, c("Gene.Name", "Chromosome", "Start", "End", "Description")]
...
genes_chrI <- dados_combinados[dados_combinados$Chromosome == "ChrI", ]
