...
arquivo_features <- "/mnt/data/AllGene_ChromosomalFeatures_with_description.tsv"

# Carregar os dados nos dataframes usando read_tsv() do tidyverse
genes <- read_tsv(arquivo_genes)
features <- read_tsv(arquivo_features)
...
genes_selecionados <- genes %>%
  select(GeneID = `Gene Name`, ORF_ID = `ORF`)

features_selecionados <- features %>%
  select(GeneID = `Gene Name`, Chromosome, Start, End, Description)
...
dados_combinados <- genes_selecionados %>%
  left_join(features_selecionados, by = "GeneID")

# Verificar a combinação
print(head(dados_combinados))

# 5. Filtragem: Genes localizados no cromossomo "ChrI" ----
genes_chrI <- dados_combinados %>%
...
contagem_por_cromossomo <- dados_combinados %>%
  group_by(Chromosome) %>%
...
dados_combinados <- dados_combinados %>%
  replace_na(list(Description = "Descrição ausente"))

print(head(dados_combinados))

# 9. Ordenação dos dados pelo comprimento do gene ----
dados_ordenados <- dados_combinados %>%
